...
function attachColorPickerListeners(colorPickers, hexInputs) {
  for (let i = 0; i < colorPickers.length; i++) {
    ...
    hexInputs[i].addEventListener("textInput", function (event) {
      ...
      let newValue =
        hexValue.substring(0, selectionStart) +
        event.data +
        hexValue.substring(selectionEnd, hexValue.length);
...
const colorSchemes = [
  {
    ...
    generateColors: function () {
      ...
      const firstLightness =
        baseLightness <= 10
          ? Math.floor(Math.random() * 11) + 90
          : Math.floor(Math.random() * 11);
      const secondLightness =
        firstLightness <= 10
          ? Math.floor(Math.random() * 11) + 90
          : Math.floor(Math.random() * 11);
      ...
      do {
        const adjustment =
          Math.random() < 0.5
            ...
            : -Math.floor(Math.random() * 5) - 3;
        modifiedSecondLightness = Math.max(
          0,
          Math.min(100, secondLightness + adjustment)
        );
  ...
  },
  {
    ...
    generateColors: function () {
      ...
      const colors = [
        ...
        `hsl(${analogousHue3}, ${baseSaturation}%, ${
  ...
  },
  {
    ...
    generateColors: function () {
      ...
      const colors = [
        ...
        `hsl(${baseHue}, ${baseSaturation}%, ${modifiedSecondLightness}%)`,
        `hsl(${complementaryHue}, ${baseSaturation}%, ${
          adjustedLightness + -10
        }%)`,
      ];
